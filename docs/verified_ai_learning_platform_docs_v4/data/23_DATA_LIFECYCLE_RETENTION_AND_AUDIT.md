...
| Problem assets, recognition evidence, and parser evidence | asset/session metadata, preprocessing derivative metadata, quality evidence, recognition job metadata, normalized recognition evidence, parse job metadata, normalized parser output, support/unsupported/review state, provider/model/prompt/schema provenance, usage/cost/latency metadata; raw and derived binaries plus raw recognition/parser provider output excluded | delete private object-storage keys, then delete `problem_sessions`/`problem_assets`; recognition and parse rows cascade with the owning problem session | pending upload expiry and backup expiry apply; raw binaries and raw provider/parser payloads are not normal export payloads |
