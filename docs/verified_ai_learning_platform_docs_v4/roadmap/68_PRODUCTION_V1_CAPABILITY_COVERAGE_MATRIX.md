...
| CAP-PRIV-001 | Account deletion, export, privacy controls | Yes | Sprint 3.7, Sprint 4.2, Sprint 4.3, Sprint 11.5 | `security/36_PRIVACY_AND_STUDENT_DATA_PROTECTION.md`, `data/23_DATA_LIFECYCLE_RETENTION_AND_AUDIT.md` | Partial | Sprint 3.7 implements account lifecycle foundations. Sprint 4.2 adds the `problemAssets` lifecycle contributor so export includes raw asset metadata, raw binaries remain excluded, and confirmed deletion removes source private object-storage keys. Sprint 4.3 extends that contributor to preprocessing derivative/evidence metadata, excludes derived binaries from export, and deletes derivative object keys. Future problem/AI/attempt/mastery/tutor stores must add contributors before completion. |
