...
import com.verifiedai.problem.domain.model.parse.ProblemParseJobStatus;
import com.verifiedai.problem.domain.model.parse.ProblemParseSource;
import com.verifiedai.problem.domain.model.parse.ProblemParseSupportStatus;
...
import com.verifiedai.problem.infrastructure.persistence.repository.ProblemParseJobJpaRepository;
...
import com.verifiedai.problem.infrastructure.persistence.repository.RecognitionEvidenceJpaRepository;
...
public class ProblemParseApplicationService {
    ...
    public ProblemParseStatusResult requestParse(
        UUID userId,
        ...
        ProblemParseJobJpaEntity job =
            transactionTemplate.execute(status -> {
                ...
                transitionToParsingIfNeeded(
                    session,
                    clock.instant()
                );

                RecognitionEvidenceJpaEntity evidence =
                    currentRecognitionEvidence(
                        userId,
                        problemSessionId
                    );

                Optional<ProblemParseJobJpaEntity> existing =
                    parseJobRepository
                ...
                ProblemParseJobJpaEntity created =
                    ProblemParseJobJpaEntity.queued(
                        UUID.randomUUID(),
                        userId,
                        problemSessionId,
                        evidence.id(),
                ...
                return parseJobRepository.saveAndFlush(
                    created
                );
            });
    ...
    public ProblemParseStatusResult getParse(
        UUID userId,
        UUID problemSessionId
    ) {
        requireActiveAccount(userId);
        ...
        return parseJobRepository
            ...
            .map(this::statusFor)
            .orElseGet(() ->
                ProblemParseStatusResult.notStarted(
                    problemSessionId
                )
            );
    }
    ...
    public int runDueParseJobs(int limit) {
        ...
        List<UUID> jobIds =
            parseJobRepository.findDueJobIds(
                clock.instant(),
    ...
    public boolean runParseJob(UUID jobId) {
        ...
        try {
            /*
             * Provider invocation remains isolated from the
             * problem-domain normalization logic.
             */
             ...
             *
             * Schema and semantic normalization continue to belong
             * to the problem module and MUST NOT move into the
             * provider-neutral AI gateway.
             */
            NormalizedProblemParse normalized =
                parseNormalizer.normalize(
    ...
    private ParseExecutionContext startExecution(
        UUID jobId
    ) {
        return transactionTemplate.execute(status -> {
                ...
                evidenceRepository
                    ...
                    .orElseThrow(() ->
                        problem(
                            HttpStatus.CONFLICT,
                            ApiErrorCode.PROBLEM_PARSE_FAILED,
                            "Recognition evidence is unavailable",
                            true,
                            "RECOGNIZE"
                        )
                    );
            ...
            if (
                ...
                throw problem(
                    HttpStatus.CONFLICT,
                    ApiErrorCode.PROBLEM_PARSE_FAILED,
                    "Recognition evidence is not eligible for parsing",
                    true,
                    "RECOGNIZE"
                );
            }
    ...
    private AiProblemNormalizeResult executeProvider(
        ParseExecutionContext context
    ) {
        /*
         * Route selection belongs to the provider-neutral AI
        ...
        /*
         * The request remains capability-specific and typed.
         *
         * It contains no provider/model selection information.
         */
        AiProblemNormalizeRequest request =
            new AiProblemNormalizeRequest(
        ...
        /*
         * The durable parse job is the idempotency boundary.
         *
         * operationId is deterministic for the concrete job attempt:
         *
         *     jobId + attemptNumber
         *
         * This means:
         *
         * - duplicate execution of the same attempt resolves to the
         *   same AI operation identity;
         * - a genuine worker retry receives a new operation identity;
         * - the durable job remains the common idempotency key.
         */
        ...
        /*
         * All provider selection, fallback, cost accounting,
         * technical provenance, usage-ledger handling and technical
         * execution policy now live behind AiModelGateway.
         */
        AiProblemNormalizeResult result =
            aiModelGateway.executeProblemNormalize(
                request,
                aiExecutionContext
            );

        /*
         * Sprint 5.1 gateway also owns this technical limit.
         *
         * Keep the existing caller-side check during the migration
         * as defense-in-depth and to preserve the established
         * Phase 4 behavior.
         *
         * Do not move semantic/schema normalization here.
         */
        if (
            ...
            throw new AiProviderException(
                ...
                false,
                "Problem parser output exceeded response limit"
            );
        }
    ...
    private void completeAccepted(
        ...
        transactionTemplate.executeWithoutResult(status -> {
            ...
            /*
             * Prevent a stale worker attempt from persisting over a
             * newer retry or already-completed execution.
             */
            if (
                !ProblemParseJobStatus.RUNNING
                    .name()
                    .equals(job.status())
                    || job.attemptCount()
                    != context.attemptNumber()
            ...
            if (
                ...
                ProblemParseJpaEntity saved =
                    parseRepository.save(
                ...
                boolean selected =
                    !ProblemParseSupportStatus.UNSUPPORTED
                        .name()
                        .equals(
                            normalized.supportStatus()
                        )
                        && selectionPolicy.shouldSelectAiParse(
                        session.currentParseId()
                    );
                ...
                if (selected) {
                    ...
                    correctionMetrics.selectionChanged(
                        ProblemParseSource.AI.name()
                    );
    ...
    private void recoverStaleRunningJobs(
        ...
        Instant staleBefore =
            clock.instant()
                .minus(
                    properties.stuckRunningTimeout()
                );
    ...
    private ProblemParseStatusResult statusFor(
        ProblemParseJobJpaEntity job
    ) {
        Optional<ProblemParseJpaEntity> parse =
            parseRepository.findByParseJobId(
                job.id()
            );
        ...
        return new ProblemParseStatusResult(
            ...
            parse
                ...
                .orElse(null),
            parse
                .map(ProblemParseJpaEntity::revision)
                .orElse(null),
            parse
                .map(
                    ProblemParseJpaEntity::supportStatus
                )
                .orElse(null),
            parse
                .map(
                    ProblemParseJpaEntity::unsupportedReason
                )
                .orElse(null),
            parse
                .map(
                    ProblemParseJpaEntity::reviewRequired
                )
                .orElse(
                    job.reviewRequired()
                ),
            ...
            parse
                .map(ProblemParseJpaEntity::provider)
                .orElse(null),
            parse
                .map(ProblemParseJpaEntity::model)
                .orElse(null),
            parse
                .map(
                    ProblemParseJpaEntity::normalizedProblemJson
                )
                .orElse(null),
            job.createdAt(),
            job.updatedAt(),
    ...
    private RecognitionEvidenceJpaEntity currentRecognitionEvidence(
        UUID userId,
        UUID problemSessionId
    ) {
    ...
    private void transitionToParsingIfNeeded(
        ProblemSessionJpaEntity session,
        Instant now
    ) {
        ProblemSessionStatus current =
            ProblemSessionStatus.valueOf(
                session.status()
            );

        if (
            current == ProblemSessionStatus.PARSING
                || current
                == ProblemSessionStatus.PARSED
        ) {
            return;
    ...
    private void transition(
        ...
        lifecyclePolicy.requireTransition(
            current,
            target
        );
        ...
        sessionMetrics.lifecycleTransition(
            current.name(),
            target.name()
        );
    }
